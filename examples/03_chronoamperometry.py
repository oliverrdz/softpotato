...
from softpotato.techniques import Chronoamperometry


def run_chronoamperometry_simulation():
    print("Setting up Chronoamperometry (CA) simulation...")

    # 1. Physical and System Parameters
    n = 1  # Number of electrons transferred
    T = 298.15  # Temperature (K)
    A = 1e-4  # Electrode area (m^2) [1 cm^2]
    D_R = 1e-9  # Diffusivity of R (m^2/s)
    D_O = 1e-9  # Diffusivity of O (m^2/s)
    ...
    k0 = 1e-4  # Standard heterogeneous rate constant (m/s)
    alpha = 0.5  # Charge transfer coefficient

    # 3. Excitation Waveform (Double-Step Chronoamperometry)
    technique = Chronoamperometry(
        E_init=0.0,  # Resting potential (V)
        t_init=0.5,  # Hold duration at initial potential (s)
        E_step1=0.4,  # First potential step (oxidation) (V)
        t_step1=2.0,  # Duration of step 1 (s)
        E_step2=-0.4,  # Second potential step (reduction) (V)
        t_step2=2.0,  # Duration of step 2 (s)
    )

    # 4. Spatial Domain (Diffusion layer thickness estimation)
    ...
    # 5. Assemble Physical Model & Kinetics Boundary Condition
    ...
    # 6. Discretizer & Time Integrator Engine
    ...
    # 8. Visualization & Figure Export
    _, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(8, 6), sharex=True, gridspec_kw={"height_ratios": [1, 2]}
    )

    # Subplot 1: Excitation Signal E(t)
    ax1.plot(result.t, result.potential, color="tab:red", lw=2)
    ax1.set_ylabel("Potential $E$ (V)")
    ax1.set_title("Chronoamperometry under Butler-Volmer Kinetics")
    ax1.grid(True, linestyle="--", alpha=0.6)

    # Subplot 2: Faradaic Current Transient i(t)
    # Convert current to microamperes (µA)
    i_microamps = result.current * 1e6
    ax2.plot(result.t, i_microamps, color="tab:blue", lw=2)
...
if __name__ == "__main__":
    run_chronoamperometry_simulation()
