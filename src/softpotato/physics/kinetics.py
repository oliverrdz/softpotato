...
class ButlerVolmerBC(BaseBoundaryCondition):
    """
    Finite-rate Butler-Volmer kinetic boundary condition for electroactive species $R$ and $O$.
    """
    ...
    def get_rate_constants(self, t: float) -> tuple[float, float]:
        """Evaluate heterogeneous anodic and cathodic rate constants (k_a, k_c) at time t."""
        E_t = self.technique(t)
        eta = E_t - self.E0

        exp_anodic = np.clip(self.alpha * self.f * eta, -500.0, 500.0)
        exp_cathodic = np.clip(-(1.0 - self.alpha) * self.f * eta, -500.0, 500.0)

        k_a = self.k0 * np.exp(exp_anodic)
        k_c = self.k0 * np.exp(exp_cathodic)

        return float(k_a), float(k_c)

    def apply(
        self, state: np.ndarray, t: float, mesh: BaseMesh, model: BaseModel
    ) -> np.ndarray:
        """
        Enforce 2nd-order surface Butler-Volmer kinetics at node 0 and bulk concentrations at node N-1.
        """
        N = mesh.num_nodes
        dx = float(mesh.dx)
        C_R = state[:N]
        ...
        k_a, k_c = self.get_rate_constants(t)

        # 2nd-order 3-point finite difference coefficients at x=0
        coeff_R = (3.0 * D_R) / (2.0 * dx)
        coeff_O = (3.0 * D_O) / (2.0 * dx)

        # Matrix system matching surface flux: J = k_a * C_R,0 - k_c * C_O,0
        M = np.array([[coeff_R + k_a, -k_c], [-k_a, coeff_O + k_c]])

        rhs = np.array(
            [
                (D_R * (4.0 * C_R[1] - C_R[2])) / (2.0 * dx),
                (D_O * (4.0 * C_O[1] - C_O[2])) / (2.0 * dx),
            ]
        )
